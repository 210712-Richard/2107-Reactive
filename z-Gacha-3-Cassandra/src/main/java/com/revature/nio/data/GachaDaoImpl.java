...
public class GachaDaoImpl implements GachaDao {
	...
	public void addGacha(GachaObject gacha) {
		...
		session.executeReactive(bound);
	}

	@Override
	public Observable<GachaObject> getGachas() {
		String query = "Select rarity, stats, name, ability, pictureUrl from gacha";
		// ReactiveResultSet is a Subject
		ReactiveResultSet rs = session.executeReactive(new SimpleStatementBuilder(query).build());

		Observable<GachaObject> gachas = Observable.fromPublisher(rs).map(row -> {
	...
	@Override
	public Future<GachaObject> getGachaByRarityAndName(Rarity rarity, String name) {
	...
	@Override
	public Observable<GachaObject> getGachasByRarity(Rarity rarity) {
